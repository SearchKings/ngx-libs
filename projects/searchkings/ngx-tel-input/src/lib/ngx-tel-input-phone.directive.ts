...
export class NgxTelInputPhoneDirective implements ControlValueAccessor {
  constructor(
    ...
    this.telInputService.setPhoneControl(this.ngControl);
  }

  ngOnInit() {
    this.ngControl.valueChanges
      .pipe(distinctUntilChanged(), startWith(this.ngControl.control.value))
      .subscribe((val) => this.telInputService.setPhone(val));
  }
  ...
  writeValue(val: string) {
    console.log('writing phone value', val);
    // this.telInputService.control.get('phone').setValue(val);
  }
